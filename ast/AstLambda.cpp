...
AstLambda *AstLambda::make(AstIdentifierList *formal, Expression *body) {
    ...
    for (vector<AstIdentifier *>::const_reverse_iterator it =
             formal->get_ids().rbegin();
         it != formal->get_ids().rend(); it++) {
        AstIdentifier *id = *it;
...
Expression *AstLambda::get_body() { return body; }

AstIdentifier *AstLambda::get_formal() { return formal; }
