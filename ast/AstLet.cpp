...
AstLet::AstLet(AstIdentifier *id, Expression *val, Expression *body) {
    ...
    this->body = body;
    this->hash_c =
        1 + id->get_hash() + 6 * val->get_hash() + 484 * body->get_hash();
}
...
string AstLet::to_value() {
    return "let " + id->to_value() + " = " + val->to_value() + " in " +
           body->to_value();
}

AstIdentifier *AstLet::get_id() { return id; }

Expression *AstLet::get_val() { return val; }

Expression *AstLet::get_body() { return body; }
