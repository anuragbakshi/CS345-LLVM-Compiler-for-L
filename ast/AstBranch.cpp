...
AstBranch::AstBranch(Expression *pred, Expression *then_exp,
                     Expression *else_exp) {
    this->et = AST_BRANCH;
    this->pred = pred;
    ...
    this->hash_c = 5 * pred->get_hash() + 2 * then_exp->get_hash() +
                   732 * else_exp->get_hash();
}
...
string AstBranch::to_value() {
    return "(if " + pred->to_value() + " then " + then_exp->to_value() +
           " else " + else_exp->to_value() + ")";
}

Expression *AstBranch::AstBranch::get_pred() { return pred; }

Expression *AstBranch::AstBranch::get_then_exp() { return then_exp; }

Expression *AstBranch::AstBranch::get_else_exp() { return else_exp; }

Expression *AstBranch::substitute(Expression *e1, Expression *e2) {
    if (e1 == this)
        return e2;
