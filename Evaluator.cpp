...
template <typename T> bool is_one_of(T v, std::initializer_list<T> vals) {
    return std::find(vals.begin(), vals.end(), v) != vals.end();
}

template <typename T> bool all_equal(std::initializer_list<T> vals) {
    return std::adjacent_find(vals.begin(), vals.end(),
                              std::not_equal_to<int>()) == vals.end();
}
...
Expression *Evaluator::eval_binop(AstBinOp *b) {
    ...
    } else if (eval_first->get_type() == AST_LIST ||
               eval_second->get_type() == AST_LIST) {
        report_error(b, "Binop @ is the only legal binop for lists");
    } else if (eval_first->get_type() == AST_NIL ||
               eval_second->get_type() == AST_NIL) {
    ...
    if (eval_first->get_type() == AST_STRING) {
        ...
        case NEQ:
            return AstInt::make(
                ...
                static_cast<AstString *>(eval_second)->get_string());
        default:
            report_error(b, "Binop " + AstBinOp::binop_type_to_string(
                                           b->get_binop_type()) +
