...
void SymbolTable::pop() {
    if (values.size() == 0) {
        cout << "You can only pop from a symbol table that you have pushed on "
                "first"
             << endl;
        assert(false);
    }
...
void SymbolTable::add(AstIdentifier *id, Expression *e) {
    if (values.size() == 0) {
        cout << "You can only add to a symbol table that you have pushed on "
                "first"
             << endl;
        assert(false);
    }
...
void SymbolTable::print_contents() {
    ...
    int i = 0;
    for (vector<map<AstIdentifier *, Expression *>>::iterator
             it = values.begin();
         it != values.end(); it++, i++) {
